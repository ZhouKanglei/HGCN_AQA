...
class msAgg(nn.Module):
    ...
    def forward(self, x, phase='train'):
        ...
        if phase == 'test':
            return out, mu, std, shot.mean(-1), scene.mean(-1), v
        else:
            return out, mu, std
...
if __name__ == '__main__':
    ...
    model = msAgg(1024, 1, num_groups=8)
    y = model(x)
